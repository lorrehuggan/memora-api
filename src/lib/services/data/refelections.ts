...
import {
  ...
  entryAnalysis,
  entryTranscript,
} from "@/services/db/schema/entry";
import { AnalyzerOutput } from "@/types/reflections";
...
export class ReflectionsService {
  ...
  static async createEntryAnalysis(data: AnalyzerOutput, entryId: string) {
    try {
      ...
      return await db.transaction(async trx => {
        ...
        if (data.relations && data.relations.length > 0) {
          for (const relation of data.relations) {
            // Make sure we have both entity IDs
            const aEntityId = relation.aEntityId ?? relation.a;
            const bEntityId = relation.bEntityId ?? relation.b;

            if (!aEntityId || !bEntityId) {
              console.warn("Skipping relation with missing entity ID");
              continue;
            }
